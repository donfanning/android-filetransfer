...
class FileIndex {
    ...
    write() {
        let target = path.join(__dirname, '../data/');
        let content = JSON.stringify(this.index);

        try {
            fs.writeFileSync(path.join(target, 'index.json'), content);
        } catch(failed) {
            this.errors.push(
                new AFTError(`FileIndex.write: ${failed.message}`)
            );
        }
    }
